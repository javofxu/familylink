 ...
 * description：烟雾报警器详情页
 */
public class SmDetailActivity extends BaseDetailActivity<ActivityDetailAlarmBinding> {
    ...
    protected void onResume() {
        ...
        if(mDevice == null){
            ...
            if(TextUtils.isEmpty(mDevice.getSubdeviceName())){
                mDBind.detailName.setText(getString(R.string.smalarm) + mDevice.getDevice_ID());
            }else{
                mDBind.detailName.setText(mDevice.getSubdeviceName());
            }
            mDBind.detailIcon.setImageResource(R.mipmap.gs530);
        }
    }
