 ...
 * description：SOS(紧急按钮)详情页
 */
public class SosDetailActivity  extends BaseDetailActivity<ActivityDetailAlarmBinding> {
    ...
    protected void onResume() {
        ...
        if(mDevice == null){
            ...
            if(TextUtils.isEmpty(mDevice.getSubdeviceName())){
                mDBind.detailName.setText(getString(R.string.soskey) + mDevice.getDevice_ID());
            }else{
                mDBind.detailName.setText(mDevice.getSubdeviceName());
            }
            mDBind.detailIcon.setImageResource(R.mipmap.gs390);
        }
    }
