 ...
 * description：CO(一氧化碳)详情页
 */
public class CoDetailActivity extends BaseDetailActivity<ActivityDetailAlarmBinding> {
        ...
        try {
            ...
            switch (draw) {
                case "11":
                    ...
                    break;
                case "55":
                    showDetail(R.drawable.device_status_normal, R.string.alarm);
                    break;
                case "AA":
                    if (qqq <= 15) {
    ...
    protected void onResume() {
        ...
        if(mDevice == null){
            ...
            if(TextUtils.isEmpty(mDevice.getSubdeviceName())){
                mDBind.detailName.setText(getString(R.string.coalarm) + mDevice.getDevice_ID());
            }else{
                mDBind.detailName.setText(mDevice.getSubdeviceName());
            }
            mDBind.detailIcon.setImageResource(R.mipmap.gs816);
        }
    }
