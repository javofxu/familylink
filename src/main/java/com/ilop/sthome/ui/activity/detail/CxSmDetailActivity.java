 ...
 * description：复合型烟感详情页
 */
public class CxSmDetailActivity extends BaseDetailActivity<ActivityDetailAlarmBinding> {
    ...
    protected void initListener() {
        ...
        mDBind.operation.setOnClickListener(view -> {
            sendSilenceSign(mDBind.operation.getText().toString().trim());
        });
    }
    ...
    private void doStatusShow(String aaa) {
        try {
            ...
            switch (draw) {
                case "17":
                    showDetail(R.drawable.device_status_alarm, R.string.cxalert7);
                    break;
                case "19":
                    showDetail(R.drawable.device_status_alarm, R.string.cxalert9);
                    mDBind.operation.setText(getResources().getString(R.string.cxalert11));
                    break;
                case "12":
                    showDetail(R.drawable.device_status_warn, R.string.cxalert2);
                    mDBind.operation.setText(getResources().getString(R.string.cxalert5));
                    break;
                case "15":
                    showDetail(R.drawable.device_status_warn, R.string.cxalert5);
                    break;
                case "1B":
                    showDetail(R.drawable.device_status_alarm, R.string.cxalert11);
                    break;
                case "AA":
                    if (qqqq <= 15) {
                        showDetail(R.drawable.device_status_warn, R.string.low_battery);
                        mDBind.operation.setText(getResources().getString(R.string.cxalert6));
                    } else {
                        showDetail(R.drawable.device_status_normal, R.string.normal);
                        mDBind.operation.setText(getResources().getString(R.string.cxalert7));
                    }
                    break;
                default:
    ...
    private void sendSilenceSign(String ds){
        try {
            if(getResources().getString(R.string.cxalert7).equals(ds)){
                if(ConnectionPojo.getInstance().testMode){
                    openTestModeAlert();
                }else {
                    mSendEquipment.sendEquipmentCommand(mDevice.getDevice_ID(),"17000000");
                }
            }else if(getResources().getString(R.string.cxalert11).equals(ds)){
                mSendEquipment.sendEquipmentCommand(mDevice.getDevice_ID(),"1B000000");
            }else if(getResources().getString(R.string.cxalert5).equals(ds)){
                mSendEquipment.sendEquipmentCommand(mDevice.getDevice_ID(),"15000000");
            }
        }catch (Exception e){
            e.printStackTrace();
        }
    ...
    protected void onResume() {
        ...
        if(mDevice == null){
            ...
            if(TextUtils.isEmpty(mDevice.getSubdeviceName())){
                mDBind.detailName.setText(getString(R.string.cxsmalarm) + mDevice.getDevice_ID());
            }else{
                mDBind.detailName.setText(mDevice.getSubdeviceName());
            }
            mDBind.detailIcon.setImageResource(R.mipmap.gs592);
        }
    }
