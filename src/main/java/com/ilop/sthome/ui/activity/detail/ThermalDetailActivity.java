 ...
 * description：热感报警器详情页
 */
public class ThermalDetailActivity extends BaseDetailActivity<ActivityDetailAlarmBinding> {
    ...
    protected void onResume() {
        ...
        if(mDevice == null){
            ...
            if(TextUtils.isEmpty(mDevice.getSubdeviceName())){
                mDBind.detailName.setText(getString(R.string.thermalalarm) + mDevice.getDevice_ID());
            }else{
                mDBind.detailName.setText(mDevice.getSubdeviceName());
            }
            mDBind.detailIcon.setImageResource(R.mipmap.gs412);
        }
    }
