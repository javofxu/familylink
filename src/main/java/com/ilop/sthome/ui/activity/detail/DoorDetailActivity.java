 ...
 * description：门磁详情页
 */
public class DoorDetailActivity extends BaseDetailActivity<ActivityDetailAlarmBinding> {
    ...
    private void doStatusShow(String aaa) {
        try {
            ...
            switch (draw) {
                case "55":
                    showDetail(R.drawable.device_status_alarm, R.string.door_open);
                    break;
                case "AA":
                    if (qqq <= 15) {
                        showDetail(R.drawable.device_status_warn, R.string.low_battery);
                    } else {
                        showDetail(R.drawable.device_status_normal, R.string.low_battery);
                        mDBind.showStatus.setText(getResources().getStringArray(R.array.door_actions)[1]);
                    }
                    break;
                case "66":
                    showDetail(R.drawable.device_status_alarm, R.string.door_still_open);
                    break;
                default:
    ...
    protected void onResume() {
        ...
        if(mDevice == null){
            ...
            if(TextUtils.isEmpty(mDevice.getSubdeviceName())){
                mDBind.detailName.setText(getString(R.string.door) + mDevice.getDevice_ID());
            }else{
                mDBind.detailName.setText(mDevice.getSubdeviceName());
            }
            mDBind.detailIcon.setImageResource(R.mipmap.gs320d);
        }
    }
