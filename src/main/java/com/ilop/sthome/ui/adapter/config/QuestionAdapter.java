...
public class QuestionAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> implements View.OnClickListener {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            View view = LayoutInflater.from(mContext).inflate(R.layout.item_questions, parent, false);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if(holder instanceof MyViewHolder){
            ((MyViewHolder) holder).nametextView.setText(datas.get(position).getQuestion());
            ((MyViewHolder) holder).contentView.setText(datas.get(position).getAnswer());
            if(datas.get(position).isopen()){
                ((MyViewHolder) holder).arrow.setImageResource(R.mipmap.arrow_down);
                ((MyViewHolder) holder).content.setVisibility(View.VISIBLE);
            }else{
                ((MyViewHolder) holder).arrow.setImageResource(R.mipmap.arrow);
                ((MyViewHolder) holder).content.setVisibility(View.GONE);
            }
            ((MyViewHolder) holder).top.setTag(position);
        }
    }
    ...
    class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(View view) {
            ...
            nametextView = view.findViewById(R.id.question);
            contentView = view.findViewById(R.id.answer);
