...
public class STEvent {
    ...
    private  int refreshevent; //刷新事件，除了协议外的；1代表刷新头部文字；2代表主界面（mainactivity）弹出progressdialog; 3代表成功获取数据后取消progressdialog;4代表自动化刷新结束;5代表刷新设备列表；6代表获取数据失败取消progressdialog;7代表网关列表弹出progressdialog;8代表切换语言跳转activity;9代表绑定FCM广播;10代表刷新温度;11代表绑定个推广播;12代表绑定华为推送广播;13代表绑定小米推送广播;
    private int serviceevent;  //服务事件，是activity向SiterService传递的 1代表后台发送同步自动化命令;2代表发送同步设备名称命令;3代表发送同步自动化命令;4代表下拉刷新发送同步自动化命令；5代表udp获取绑定设备的信息（devid和bindkey）;6代表开始搜索局域网；7代表网络信号变化广播；8.代表网关切换后台命令
