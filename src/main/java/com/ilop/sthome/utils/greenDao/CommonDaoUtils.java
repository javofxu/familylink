...
public class CommonDaoUtils <T>
{
     ...
     * @return
     */
    public boolean deleteByQuery(WhereCondition cond, WhereCondition... condMore)
    {
        try
        {
            //按照id删除
            daoSession.queryBuilder(entityClass).where(cond, condMore).buildDelete().executeDeleteWithoutDetachingEntities();
            return true;
        }
        catch (Exception e)
        {
